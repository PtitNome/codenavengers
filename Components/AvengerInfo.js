...
class AvengerInfo extends React.Component {
  ...
  render() {
    /* On récupère les objets avenger et thumbnail passés par les props.navigation.state.params
      en provenance du Navigateur */
    const { avenger } = this.props.navigation.state.params
    const { thumbnail } = this.props.navigation.state.params.avenger
    return (
      <View style={styles.main_container}>
        <View style={styles.view_container}>
          <Image
            style={styles.avg_img}
            source={ getAvengerImage( thumbnail ) }
          />
        </View>
        <ScrollView style={styles.scrollview_container}>
          <Text style={styles.text_name}>Name : { avenger.name }</Text>
          <Text style={styles.default_text}>Real identity : Lorem Ipsum</Text>
          <Text style={styles.default_text}>Age : 38</Text>
          <Text style={styles.description_text}>Description : { avenger.description }</Text>
